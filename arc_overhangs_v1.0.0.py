...
"""
A list of pairs that allows the script to recognize what slicer is used by some specific comment
(or section of comment) that is unique to the slicer. The comment will most likely include the name
of that slicer.

Should you like to add another slicer, you must also add its config block indicator in
_CONFIG_BLOCKS, all settings found in _SLICER_SETTINGS_MAP, and all annotations found
in _EQUIVALENT_NAMES.

- Key (left side of the tuple): comment that is unique to the slicer.
- Value (right side of the tuple): the name of the slicer.
"""
_SLICER_INDICATORS = (
    ("; generated by PrusaSlicer", "PrusaSlicer"),
    ("; generated by OrcaSlicer", "OrcaSlicer"),
    ("; generated by SuperSlicer", "SuperSlicer"),
    # TODO Add indicators for other slicers
)

def detect_slicer(line: str) -> str:
    """Return the name of the slicer if it can be detected from the given line of GCode."""
    for key, slicer in _SLICER_INDICATORS:
        if key in line:
            return slicer
...
_CONFIG_BLOCKS = (
    ...
    # TODO Add annotations for other slicers
)
...
_SLICER_SETTINGS_MAP = {
    ...
    'SuperSlicer': {
        ...
        "external_perimeters_first": "external_perimeters_first",
        'extrusion_width': 'extrusion_width',
        'extrusion_spacing': 'extrusion_width', # There are two options for extrusion width/spacing in SuperSlicer. Only one can exist at once.
        "filament_diameter": "filament_diameter",
        "infill_extrusion_width": "infill_extrusion_width",
        "infill_extrusion_spacing": "infill_extrusion_width", # There are two options for infill extrusion width/spacing in SuperSlicer. Only one can exist at once.
        "infill_first": "infill_first",
        "layer_height": "layer_height",
        'nozzle_diameter': 'nozzle_diameter',
        'overhangs_width_speed': 'overhangs',
        'perimeter_extrusion_width': 'perimeter_extrusion_width',
        'perimeter_extrusion_spacing': 'perimeter_extrusion_width', # There are two options for perimeter extrusion width/spacing in SuperSlicer. Only one can exist at once.
        "retract_length": "retract_length",
        'retract_speed': 'retract_speed',
        "solid_infill_extrusion_width": "solid_infill_extrusion_width",
        'solid_infill_extrusion_spacing': 'solid_infill_extrusion_width', # There are two options for solid infill extrusion width/spacing in SuperSlicer. Only one can exist at once.
        'temperature': 'temperature',
        'travel_speed': 'travel_speed',
        'use_relative_e_distances': 'use_relative_e_distances',
    },

    # TODO Add mappings for other slicers here
}
...
class Layer():
    ...
    def makeExternalPerimeter2Polys(self) -> None:
        ...
        for idf, fe in enumerate(self.features):
            ...
            if extPerimeterIsStarted and (idf == len(self.features) - 1 or not (getSlicerSpecificName(";TYPE:External perimeter") in ftype or getSlicerSpecificName(";TYPE:Overhang perimeter") in ftype or (slicer=="SuperSlicer" and ";TYPE:Thin wall" in ftype))):
